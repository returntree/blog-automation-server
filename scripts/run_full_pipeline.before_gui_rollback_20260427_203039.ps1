...
if (-not [string]::IsNullOrWhiteSpace($env:BLOG_AUTOMATION_RUN_LOG_PATH)) {
    $runLogPath = $env:BLOG_AUTOMATION_RUN_LOG_PATH
} else {
    $runLogPath = Join-Path $logsDir "run_full_pipeline_$timestamp.log"
}
...
function Show-GuiProgressWindow {
    if ($env:BLOG_AUTOMATION_GUI -ne "1") { return }
    $title = $env:BLOG_AUTOMATION_GUI_WINDOW_TITLE
    if ([string]::IsNullOrWhiteSpace($title)) { $title = "블로그 자동화 v8 실행" }
    try {
